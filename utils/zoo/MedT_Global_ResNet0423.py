...
sys.path.append(r'D:\Programming\AI&ML\MainResearch\utils')
import utils
import time
'''
2022/4/23設計之模型。
"MedT_Global_ResNet0423.py"
"MedT_Global_ResNet0423.py"
"MedT_Global_ResNet0423.py"
"MedT_Global_ResNet0423.py"

模型相關說明：
Global branch 可以導入ResNet框架的模型。
另外Local branch 加入速度優化過後的架構(channel 融合 patches)
...
class use_global_branch(nn.Module):
    def __init__(self, module_name, pretarin=False, s=0.125):
        ...
        if module_name == 'resnet34':
            self.model = smp.UnetPlusPlus(module_name, in_channels=8, classes=16, encoder_weights=pretarin)
            self.upsample = nn.Upsample(scale_factor=2)
        if module_name == 'resnet50':
            self.model = smp.UnetPlusPlus(module_name, in_channels=8, classes=16, encoder_weights=pretarin)
            self.upsample = nn.Upsample(scale_factor=2)
        if module_name == 'resnet101':
            self.model = smp.UnetPlusPlus(module_name, in_channels=8, classes=16, encoder_weights=pretarin)
            self.upsample = nn.Upsample(scale_factor=2)
    ...
    def forward(self, x):
        ...
        if self.modules_name == 'resnet50':
            x = self.model(x)
            x = self.upsample(x)
            return x

        if self.modules_name == 'resnet101':
            x = self.model(x)
            x = self.upsample(x)
            return x
