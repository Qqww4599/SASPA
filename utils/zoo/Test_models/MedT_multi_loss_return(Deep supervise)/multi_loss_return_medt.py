...
class medt_retrofit_model(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=1, zero_init_residual=True,
        ...
        # self.decoder5 = nn.Conv2d(64, int(128 * s), kernel_size=3, stride=1, padding=1)
        ...
        if self.global_cnn is not None:
            self.global_encoder1 = global_cnn(8, out_plane=32)  # dim 8 -> 32
            self.global_encoder2 = global_cnn(32, out_plane=64, downsample=True)

            self.feature_conv = nn.Sequential(nn.Conv2d(64, 2, bias=False, kernel_size=1),
                                               nn.Upsample(scale_factor=(4,4)))

            self.global_decoder2 = global_cnn(64, out_plane=32, upsample=True)
            self.global_decoder1 = global_cnn(32, out_plane=16, upsample=True)
    ...
    def _forward_impl(self, x):
        ...
        # x = self.maxpool(x)
        x1 = self.global_encoder1(x) # 8 -> 32 , (b, 32, 64, 64)
        x2 = self.global_encoder2(x1) # 32 -> 64, downsample=(b,64,32,32)
        out_feature = self.feature_conv(x2)
        x = self.global_decoder2(x2)
        x = torch.add(x, x1)
        x = self.global_decoder1(x)
        ...
        def patch_attention(patches, xin):
            ...
            return x_loc
        x_loc = patch_attention(16, xin) # b, 16, 128, 128
...
class use_global_branch(nn.Module):
    def __init__(self, module_name, pretarin=False, s=0.125):
        ...
        if module_name == 'self_def_cnn':
            ...
            self.decoder4 = nn.Conv2d(int(512 * s), int(256 * s), kernel_size=3, stride=1, padding=1)
            self.decoder5 = nn.Conv2d(int(256 * s), int(128 * s), kernel_size=3, stride=1, padding=1) # 32 -> 16
...
if __name__ == '__main__':
    imgsize = 128
    import argparse
    parser = argparse.ArgumentParser(description='Multiloss Return Model')
    ...
    def model_test():
        # 只測試model
        test_input = torch.randn(16, 3, imgsize, imgsize).cuda()
        model = medt_retrofit_model_use(args).cuda()
        output, feature = model(test_input)
        print('model_test_output:',output.shape)
        print('model_feature_output:',feature.shape)
