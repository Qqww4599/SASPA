import sys
sys.path.append(r'D:\Programming\AI&ML\MainResearch\utils\zoo\Test_models\TransCycle_model\axial_attention\reversible.py')
import torch
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, kv = None):
        ...
        # print(q.shape,k.shape, v.shape) # torch.Size([1, 64, 64, 64]) torch.Size([1, 64, 64, 64]) torch.Size([1, 64, 64, 64])
...
if __name__ == '__main__':
    m = SelfAttention(dim=16, heads=8,dim_heads=8)
    sao_m2 = AxialAttention(dim=1, heads=1, dim_index=1, sum_axial_out = True)
    no_sao_m2 = AxialAttention(dim=1, heads=1, dim_index=1, sum_axial_out = False)
    x = torch.randn(1,32*32,16) # b,c,h,w -> b,n,c
    x2 = torch.randn(1,1,8,8)
    # print(m(x).shape)
    print(f'sum_axial_out {sao_m2(x2)}, NO sum_axial_out {no_sao_m2(x2)}')
