...
# -- 作為模型訓練時需要將此處打開 --
from . import triple_attention as ta
from torch.utils.checkpoint import checkpoint
from . import axiel_attention_blocks as aab
from . import Swin_transformer_unet_expand_decoder_sys as swin
from einops import rearrange, reduce
from .global_branch.global_branch_source import global_branch
from .global_branch.triple_attention import TripletAttention
...
class medical_transformer(nn.Module):
    def __init__(self, block, block2, layers, num_classes, groups, width_per_group,
        ...
        self.layer1_p = self._make_layer(block2, int(128*s), layers[0], kernel_size=img_size_p//2) # 第一層輸出尺寸不變
        self.layer2_p = self._make_layer(block2, int(256*s), layers[1], stride=2, kernel_size=img_size_p//2,
                                         dilate=relpace_stride_with_dilation[0]) # 尺寸減半，通道數倍增
        self.layer3_p = self._make_layer(block2, int(512*s), layers[2], stride=2, kernel_size=img_size_p//4,
                                         dilate=relpace_stride_with_dilation[1]) # 尺寸減半，通道數倍增
        self.layer4_p = self._make_layer(block2, int(1024*s), layers[3], stride=2, kernel_size=img_size_p//8,
    ...
    def _BCHW_to_BNC(self, x, inplanes, size ,reverse=False):
        ...
        if not reverse:
            ...
            assert x.shape[1:] == (inplanes, size, size), '輸入尺寸應為(B,{},{},{}), 但現在輸入形狀為{}'.format(inplanes,
                                                                                                   size,
                                                                                                   size,
                                                                                                   x.shape,
                                                                                                   )
            ...
            H,W = size,size
            assert x.shape[1:] == (size*size,inplanes), '輸入尺寸應為(B,{},{}), 但現在輸入形狀為{}'.format(size*size,
                                                                                                   inplanes,
                                                                                                   x.shape,
                                                                                                   )
            # 為何self.inplanes在forward裡面調用會是256而不是8?256從哪裡來?
            # 因為在init進行axialatten_block就會將self.inplane放大了
            B,N,C = x.shape
    ...
    def _forward_impl(self, x):
        ...
        # # x2 = self._BCHW_to_BNC(x2, inplanes=int(512*0.125), size=self.img_size//4)
        # # x2 = self.swin_block(x2)
        # # x2 = self._BCHW_to_BNC(x2, inplanes=int(512*0.125), size=self.img_size//4 ,reverse=True)
        #
        # x = F.interpolate(self.decoder4(x2), scale_factor=2, align_corners=True, mode='bilinear')
        # x = torch.add(x, x1)
        # x = F.interpolate(self.decoder5(x), scale_factor=2, align_corners=True, mode='bilinear')
    ...
    def _local_attention(self, patches, xin, x_loc):
        ...
        for i in range(0, H_len):
            for j in range(0, H_len):
                ...
                x_p = F.relu(F.interpolate(self.decoder1_p(x4_p), scale_factor=(2, 2), mode='bilinear', align_corners=False))
        ...
        return x_loc
    def _local_attention_ver2(self, patches, xin, x_loc):
        '''
...
def main(x):
    ...
    parser.add_argument('--imgsize', type=int, default=128, help='圖片大小')
    parser.add_argument('--imgchan', type=int, default=3, help='model輸入影像通道數(grayscale)')
    parser.add_argument('--classes', type=int, default=1, help='model輸出影像通道數(grayscale)')
    args = parser.parse_args()

    model = medt(args)

    out = model(x)
    out = out[:,0,:,:].squeeze(0).detach().numpy()
...
if __name__ == '__main__':
    # test = torch.arange(0,128*128*3).reshape(1,3,128,128) / 128*128*3
