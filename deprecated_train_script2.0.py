...
from utils.Other_utils import remove_readonly
...
def main(args):
    ...
    if args.dataset == 'GS':
        ...
        # train_tf = JointTransform2D()
        # train_dataset = ImageToImage2D(args.train_dataset,img_size=(args.imgsize,args.imgsize),joint_transform=train_tf)
    ...
    # loss分配比例。使用adv_model時會使用到
    final_loss_p, loss1_p, loss2_p, loss3_p = args.loss_proportion
    ...
    def train_epoch(test_epoch, safe_mode=1):
        ...
        def train_one_epoch(dataloader, model):
            ...
            for i, (image, mask) in enumerate(dataloader):
                ...
                with autocast(enabled=args.use_autocast):
                    final_output, loss_1, loss_2, loss_3 = model(image)  # b,c,h,w
                    # loss計算
                    final_output = choose_loss_fn(final_output, mask)
                    loss_1 = choose_loss_fn(loss_1, mask)
                    loss_2 = choose_loss_fn(loss_2, mask)
                    loss_3 = choose_loss_fn(loss_3, mask)


                    total_loss = final_loss_p*final_output + loss1_p*loss_1 + loss2_p*loss_2 + loss3_p*loss_3
                    loss = total_loss / train_accumulation_steps
                # =================backward==================
        ...
        def eval(val_dataset, model, epoch, binarization=False, scaling=False):
            ...
            for i, (original_image, mask, original_size) in enumerate(val_dataset):
                x = original_image.to(device)
                with no_grad():
                    pred, *others = model(x) # b,c,h,w
                    # pdb.set_trace()
                    # pred_ori = pred.clone()
                    # Use loss function
                    if scaling:
                    ...
                    pred = pred.to('cpu') # 計算loss在cpu進行
                    test_loss += choose_loss_fn(pred, mask)
                    f1 += loss_fn.classwise_f1(pred, mask)
                    iou += loss_fn.classwise_iou(pred, mask)
                # print('pred.shape：',pred.shape)
                Save_image(original_image, pred, mask, save_path=fr'{save_path}/num{i + 1}',
                           original_size=original_size
                           , channel=args.imgchan, th=int(args.threshold * 255))
            val_loss = test_loss / len(val_dataset)
            f1 = f1 / len(val_dataset)
            iou = iou / len(val_dataset)
            iou_test = iou_test / len(val_dataset)
            print(f'epoch {epoch} validation.','avg_eval_loss：{:.4f}, f1 score：{:.4f}, mIoU score：{:.4f}'.format(test_loss,f1.item(),iou.item()),
                  sep='\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scheduler', type=str, default='CosineAnnealingLR', help='使用的scheduler')
    ...
    parser.add_argument('--loss_proportion', type=tuple, default=(0.5, 0.3, 0.2, 0.1), help='adv_model使用的回傳loss分配，'
                                                                                           '對應依序為:'
                                                                                            'final_output:最終輸出結果'
                                                                                            'loss_1:ResNet branch輸出結果'
                                                                                            'loss_2:Attention global branch輸出結果'
                                                                                            'loss_3:tiny_branch+ResNet低維度輸出結果'
                                                                                           '。總和必須為1')
